...
namespace OpenGlobe.Renderer
{
    public class Camera
    {
        ...
        public Vector3F EyeHigh => Eye.ToVector3F();

        public Vector3F EyeLow => (Eye - EyeHigh.ToVector3D()).ToVector3F();

        public Vector3D Forward => (Target - Eye).Normalize();

        public Vector3D Right => Forward.Cross(Up).Normalize();

        public double FieldOfViewX => (2.0 * Math.Atan(AspectRatio * Math.Tan(FieldOfViewY * 0.5)));
        ...
        public double OrthographicDepth => Math.Abs(OrthographicFarPlaneDistance - OrthographicNearPlaneDistance);
