...
namespace OpenGlobe.Renderer
{
    public class SceneState
    {
        ...
        public Vector3D CameraLightPosition => Camera.Eye;

        public Matrix4D ComputeViewportTransformationMatrix(Avalonia.PixelRect viewport, double nearDepthRange, double farDepthRange)
        {
            ...
            //
            return new Matrix4D(
                halfWidth, 0.0,        0.0,       viewport.X + halfWidth,
                0.0,       halfHeight, 0.0,       viewport.Y + halfHeight,
                0.0,       0.0,        halfDepth, nearDepthRange + halfDepth,
                0.0,       0.0,        0.0,       1.0);
        }

        public static Matrix4D ComputeViewportOrthographicMatrix(Avalonia.PixelRect viewport)
        {
            //
            // Bottom and top swapped:  MS -> OpenGL
            //
        ...
        public Matrix4D OrthographicMatrix
            ...
            // Bottom and top swapped:  MS -> OpenGL
            //
            => Matrix4D.CreateOrthographicOffCenter(Camera.OrthographicLeft, Camera.OrthographicRight,
                    Camera.OrthographicTop, Camera.OrthographicBottom,
                    Camera.OrthographicNearPlaneDistance, Camera.OrthographicFarPlaneDistance);

        public Matrix4D PerspectiveMatrix => Matrix4D.CreatePerspectiveFieldOfView(Camera.FieldOfViewY, Camera.AspectRatio,
                    Camera.PerspectiveNearPlaneDistance, Camera.PerspectiveFarPlaneDistance);

        public Matrix4D ViewMatrix => Matrix4D.LookAt(Camera.Eye, Camera.Target, Camera.Up);

        public Matrix4D ModelMatrix { get; set; }

        public Matrix4D ModelViewMatrix => ViewMatrix * ModelMatrix;
        ...
        public Matrix4D ModelViewPerspectiveMatrixRelativeToEye => PerspectiveMatrix * ModelViewMatrixRelativeToEye;

        public Matrix4D ModelViewPerspectiveMatrix => PerspectiveMatrix * ModelViewMatrix;

        public Matrix4D ModelViewOrthographicMatrix => ModelViewMatrix * OrthographicMatrix;
