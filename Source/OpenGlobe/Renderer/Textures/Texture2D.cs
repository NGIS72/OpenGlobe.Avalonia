...
namespace OpenGlobe.Renderer
{
    public abstract class Texture2D : Disposable
    {
        ...
        private void SaveColor(string filename)
        {
            ...
            {
                Bitmap bitmap = pixelBuffer.CopyToBitmap(Description.Width, Description.Height, SKColorType.Rgba8888);
        ...
        private void SaveFloat(string filename, ImageFormat imageFormat)
        {
            using (ReadPixelBuffer pixelBuffer = CopyToBuffer(imageFormat, ImageDatatype.Float, 1))
            {
                ...
                using (Bitmap bitmap = new Bitmap(Description.Width, Description.Height, SKColorType.Rgba8888, SKAlphaType.Premul))
                using (var stream = new MemoryStream())
                {
                    int j = 0;
                    for (int y = Description.Height - 1; y >= 0; --y)
                    {
                        for (int x = 0; x < Description.Width; ++x)
                        {
                            ...
                            byte intensity = (byte)(linearDepth * 255.0f);
                            bitmap.SetPixel(x, y, new SKColor(intensity, intensity, intensity, (byte)255));
                        }
                    }
                    bitmap.Encode(stream, SKEncodedImageFormat.Png, 100);
                    File.WriteAllBytes(filename, stream.ToArray());
