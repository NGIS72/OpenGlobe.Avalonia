...
namespace OpenGlobe.Scene
{
    internal class ClipmapUpdater : IDisposable
    {
        ...
        public ClipmapUpdater(Context context, ClipmapLevel[] clipmapLevels)
        {
            ...
            _updateShader = m_Context.Device.CreateShaderProgram(
            ...
            _upsampleShader = m_Context.Device.CreateShaderProgram(
            ...
            _computeNormalsShader = m_Context.Device.CreateShaderProgram(
        ...
        private void RenderTileToLevelTexture(Context context, ClipmapLevel level, RasterDataDetails details, RasterTileRegion region, LazyTexture2D tileTexture)
        {
            ...
            context.TextureUnits[0].Texture = tileTexture.Get(context);
            ...
            context.Viewport = new Avalonia.PixelRect(0, 0, levelTexture.Description.Width, levelTexture.Description.Height);
        ...
        private void UpsampleTileData(Context context, ClipmapLevel level, RasterDataDetails details, RasterTileRegion region)
        {
            ...
            context.TextureUnits[0].TextureSampler = m_Context.Device.TextureSamplers.LinearRepeat;
        ...
        private void UpdateNormals(Context context, ClipmapUpdate update)
        {
            ...
            context.TextureUnits[0].TextureSampler = m_Context.Device.TextureSamplers.NearestRepeat;
            ...
            // Update the context and draw
            context.Viewport = new Avalonia.PixelRect(0, 0, level.NormalTexture.Description.Width, level.NormalTexture.Description.Height);
            context.Framebuffer = _framebuffer;
        ...
        private void RequestThreadEntryPoint(object state)
        {
            ...
            while (true)
            {
                ...
                if (request != null)
                {
                    ...
                    Fence fence = m_Context.Device.CreateFence();
                    fence.ClientWait();
        ...
        private class RasterDataDetails
        {
            ...
            public Dictionary<RasterTileIdentifier, LazyTexture2D> LoadedTiles = new Dictionary<RasterTileIdentifier, LazyTexture2D>();
