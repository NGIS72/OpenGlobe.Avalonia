    ...
    public class GlobeClipmapTerrain : IRenderable, IDisposable
    {
        ...
        public GlobeClipmapTerrain(Context context, RasterSource terrainSource, EsriRestImagery imagery, Ellipsoid ellipsoid, int clipmapPosts)
        {
            ...
            for (int i = 0; i < _clipmapLevels.Length; ++i)
            {
                ...
                _clipmapLevels[i].HeightTexture = m_Context.Device.CreateTexture2D(new Texture2DDescription(_clipmapPosts, _clipmapPosts, TextureFormat.Red32f));
                _clipmapLevels[i].NormalTexture = m_Context.Device.CreateTexture2D(new Texture2DDescription(_clipmapPosts, _clipmapPosts, TextureFormat.RedGreenBlue32f));
                ...
                _clipmapLevels[i].ImageryTexture = m_Context.Device.CreateTexture2D(new Texture2DDescription(_clipmapLevels[i].ImageryWidth, _clipmapLevels[i].ImageryHeight, TextureFormat.RedGreenBlue8, false));
            }

            _shaderProgram = m_Context.Device.CreateShaderProgram(
        ...
        private bool RenderLevel(int levelIndex, ClipmapLevel level, ClipmapLevel coarserLevel, bool fillRing, Vector2D center, Context context, SceneState sceneState)
        {
            ...
            context.TextureUnits[1].TextureSampler = m_Context.Device.TextureSamplers.LinearRepeat;
            context.TextureUnits[2].Texture = level.NormalTexture;
            context.TextureUnits[2].TextureSampler = m_Context.Device.TextureSamplers.LinearRepeat;
