...
namespace OpenGlobe.Scene
{
    ...
    {
        public static TerrainTile FromBitmap(string filename)
        {
            return FromBitmap(Bitmap.Decode(filename));
        }
        public static TerrainTile FromBitmap(Bitmap bitmap)
        {
            ...
            for (int j = bitmap.Height - 1; j >= 0; --j)
            {
                ...
                {
                    float height = (float)(bitmap.GetPixel(i, j).Red / 255.0);
                    heights[k++] = height;
        ...
        public RectangleD Extent => _extent;
